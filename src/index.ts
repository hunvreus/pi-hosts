...
function flagValue(pi: ExtensionAPI, name: string, fallback: string): string {
  const value = pi.getFlag(name);
  return typeof value === "string" && value.trim() ? value : fallback;
}

export default function registerPiHosts(pi: ExtensionAPI): void {
  ...
  const state: TurnState = { activeHostId: null, turnHostIds: [] };
