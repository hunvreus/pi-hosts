...
import type { ImportMode } from "../import-ssh-config.js";
...
function mode(args: string[]): ImportMode {
  const raw = option(args, "--mode");
  if (raw === "preview" || raw === "create_only" || raw === "update_transport" || raw === "all" || raw === "aliases") {
    return raw;
  }
  if (has(args, "--preview")) return "preview";
  if (has(args, "--update-transport")) return "update_transport";
  if (has(args, "--aliases")) return "aliases";
  return "create_only";
}

export async function handleHostsCommand(raw: string, ctx: ExtensionCommandContext, config: CommandConfig): Promise<void> {
  ...
  if (group === "config") {
    ...
    if (action === "reload") {
      ...
      ctx.ui.notify(
        `reloaded=${loaded.path ?? "(defaults only)"}\napproval=${loaded.config.policy.approval}\nbackend=${loaded.config.policy.backend}`,
        "info",
      );
      return;
    }
  ...
  if (group === "import" && action === "ssh") {
    ...
    if (mode(args) !== "preview") saveStore(config.hostsPath, store);
    ctx.ui.notify(
