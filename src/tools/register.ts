...
import type { RiskAssessment, TurnState } from "../types.js";
...
function requested(values: unknown): string[] {
  return Array.isArray(values) ? values.map(String) : [];
}

function auditReject(config: ToolConfig, hosts: string[], command: string, risk: RiskAssessment, durationMs: number): void {
...
export function registerTools(pi: ExtensionAPI, state: TurnState, config: ToolConfig): void {
  pi.registerTool({
    name: "hosts_list",
    label: "Hosts List",
  ...
  pi.registerTool({
    ...
    async execute(_id, params) {
      ...
      if (removed) saveStore(config.hostsPath, store);
  ...
  pi.registerTool({
    ...
    async execute(_id, params) {
      ...
      if (mode !== "preview") saveStore(config.hostsPath, store);
      return text(
        `created=${result.created.length} updated=${result.updated.length} skipped=${result.skipped.length} preview=${result.preview.length}`,
        result,
      );
    },
  });
  ...
  pi.registerTool({
    ...
    async execute(_id, params) {
      ...
      if (targets.length === 0) return text("No hosts resolved for facts refresh.");
      const lines: string[] = [];
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      const store = loadStore(config.hostsPath);
      const targets = resolveTargets(store, requested(params.hosts), state);
      if (targets.length === 0) return text("No target hosts resolved. Provide hosts or mention a known #host.");
  ...
  pi.registerTool({
    ...
    async execute(_id, params) {
      ...
      if (!host) return text(`Host not found: ${params.host}`);
