...
function host(value) {
    ...
    return {
        ...
        bastionHostId: typeof value.bastionHostId === "string" || value.bastionHostId === null ? value.bastionHostId : undefined,
...
export function loadStore(path) {
    try {
        ...
        if (!isRecord(parsed) || parsed.version !== 1 || !Array.isArray(parsed.hosts))
            return emptyStore();
        return { version: 1, hosts: parsed.hosts.map(host).filter((item) => item !== null) };
    }
    catch (error) {
